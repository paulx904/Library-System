  ...
  <body>
	...
	<div style="width:1350px;height:70px;margin:auto">
		...
		<?php 			
			...
			//若輸入資訊完整，則執行return動作；否則，alert使用者 			
			if (isset($_POST['checkbox'])){ 				
				$sql = "UPDATE book SET returndate = ? WHERE bid = ?"; 	//填上returndate，且將此借閱紀錄INSERT INTO records資料表		
				foreach($_POST['checkbox'] as $index){ 					
					$statement= $dbgo->prepare($sql); 					
					$statement->execute([date("Y-m-d H:i:s"), $index]);
					$sql_insert = "INSERT INTO records SELECT null, uid, bid, startdate, duedate, returndate FROM book WHERE bid = $index";
					$statement = $dbgo->prepare($sql_insert); 			
					$statement->execute();
					$sql_clear = "UPDATE book SET status = 1, startdate = null, duedate = null, returndate = null, uid = 0 WHERE bid = $index"; 					
					$statement = $dbgo->prepare($sql_clear); 			 					
					$statement->execute();
				} 				
				echo "<script>alert('成功還書!')</script>";
			} 			
			else 				
				echo "<script>alert('請勾選要歸還的書籍!')</script>";
			
			//以表格顯示此用戶尚未歸還之書的資訊 			
			$statement = $dbgo->prepare("SELECT * FROM book, user WHERE book.uid = user.uid && user.uname = ? && book.status = 0 ORDER BY bid ASC");
			...
			echo "</table><br>"; 			
			echo "<button class='btn btn-primary' type='submit'>歸還</button><br><br>"; 			
			echo "</form>";
		?>
